...
- When applicable, the Copilot Cloud agent must start the application through the Aspire AppHost and directly test its modifications (for example, by exercising the affected API endpoint or browser workflow). Report the validation performed; if startup or direct testing is blocked, report the exact command and environment limitation.
